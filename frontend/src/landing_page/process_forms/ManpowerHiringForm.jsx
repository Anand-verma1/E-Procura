...
const ESSENTIAL_QUALS = {
  ...
  manager:    ["ME/MTech in CS/IT or relevant discipline", "BE/BTech in CS/IT with 3+ years experience", "MSc/MCA with 3+ years relevant experience"],
  engineer:   ["BE/BTech in CS/IT or relevant discipline", "Diploma in CS/IT with 3+ years experience"],
  internship: ["Currently pursuing BE/BTech, MSc, or MCA in relevant discipline", "Currently pursuing PhD in relevant discipline"],
};
...
const AGE_MAP     = { postdoc: "50", associate: "45", jrf: "35", manager: "45", engineer: "40", internship: "30" };
const SUBJECT_MAP = { postdoc: "MeiTy-PostDoc", associate: "MeiTy-ProjectAssociate/SRF", jrf: "MeiTy-ProjectAssistant/JRF", manager: "MeiTy-ProjectManager", engineer: "MeiTy-ProjectEngineer", internship: "MeiTy-Internship" };
...
const readonlyCls = "w-full border border-gray-200 rounded-lg px-4 py-2 text-sm text-gray-700 bg-gray-50 cursor-not-allowed";
...
export default function ManpowerHiringForm({ projectId = "", projectCode: initProjCode = "", projectTitle: initProjTitle = "", piName: initPiName = "", piEmail: initPiEmail = "" }) {
  ...
  const setTermLine = (i, v) => setCustomTerms((p) => { const c = [...p]; c[i] = v; return c; });
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Field label="Date of Request" required>
                <input type="date" value={reqDate} readOnly className={readonlyCls} />
              </Field>
              <Field label="Project Code">
                <input value={projCode} readOnly placeholder="Auto-fetched from project" className={readonlyCls} />
              </Field>
              <div className="md:col-span-2">
                <Field label="Project Title" required>
                  <textarea value={projTitle} readOnly rows={2}
                    placeholder="Auto-fetched from project" className={readonlyCls + " resize-none"} />
                </Field>
              </div>
              <div className="md:col-span-2">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Field label="Full Name" required>
                <input value={piName} readOnly placeholder="Auto-fetched from profile" className={readonlyCls} />
              </Field>
              <Field label="Email ID" required>
                <input type="email" value={piEmail} readOnly placeholder="Auto-fetched from profile" className={readonlyCls} />
              </Field>
              <div className="md:col-span-2">
      ...
      {step === 3 && (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6">
            ...
            <p className="text-xs text-gray-400 mb-4">These terms are mandatory and will be included in the hiring notice.</p>
            <ol className="list-decimal list-outside ml-4 space-y-2 mb-6">
              {STANDARD_TERMS.map((term, i) => (
                <li key={i} className="text-sm text-gray-700 leading-relaxed">{term}</li>
              ))}
            </ol>
            <div className="border-t border-gray-100 pt-4">
              <p className="text-xs text-gray-500 font-semibold mb-2">Additional Terms (optional)</p>
              <div className="space-y-2">
                {customTerms.map((line, i) => (
                  <div key={i} className="flex gap-2">
                    <input value={line} onChange={(e) => setTermLine(i, e.target.value)}
                      placeholder={`Additional term ${i + 1}...`} className={inputCls + " text-sm"} />
                    {customTerms.length > 1 && (
                      <button onClick={() => delTermLine(i)} className="text-red-400 hover:text-red-600 text-lg leading-none shrink-0">✕</button>
                    )}
                  </div>
                ))}
              </div>
              <button onClick={() => setCustomTerms((p) => [...p, ""])}
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6">
            ...
            <p className="text-sm text-gray-500 mb-4">Upload the application form template that candidates will fill and submit.</p>
            <label className={`flex flex-col items-center justify-center gap-2 border-2 border-dashed rounded-xl p-8 cursor-pointer transition
              ...
              {appFormFile ? (
                <>
                  ...
                  <p className="text-sm font-semibold text-blue-700">{appFormFile.name}</p>
                  <p className="text-xs text-gray-400">{(appFormFile.size / 1024).toFixed(1)} KB — click to change</p>
                </>
              ) : (
                <>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8">
              {[
                ...
                ["Application Form",   appFormFile ? appFormFile.name : "Not uploaded"],
              ].map(([k, v], i) => (
          ...
          {submitted ? (
            <div className="bg-green-50 border border-green-200 rounded-2xl p-6 text-center">
              ...
              <p className="text-green-700 font-semibold text-lg">Request submitted successfully</p>
              <p className="text-sm text-gray-500 mt-1">The hiring request has been saved and is pending review.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
